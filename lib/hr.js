...
// Company / authorised-signatory signature — applied to the company execution
// block on every generated contract. Uploaded by a full HR admin from Settings.
export const uploadCompanySignature = async (file) => {
  if (!file) throw new Error('No file');
  const ext = (file.name?.split('.').pop() || 'png').toLowerCase();
  const path = `company/signature-${Date.now()}.${ext}`;
  const ref = sRef(storage, path);
  const task = uploadBytesResumable(ref, file);
  await task;
  const url = await getDownloadURL(task.snapshot.ref);
