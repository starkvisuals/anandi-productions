...
export const ensurePrimaryProducerExists = async (currentUserProfile) => {
  ...
  // Only act if the current user IS the admin AND isn't already flagged.
  if (currentUserProfile.id !== adminId) return { updated: false, reason: 'not-owner' };
  if (currentUserProfile.isPrimaryProducer === true) return { updated: false, reason: 'already-set' };

  // Defensive: also check the Firestore doc in case userProfile is stale
  const userSnap = await getDoc(doc(db, 'users', adminId));
  if (!userSnap.exists()) return { updated: false, reason: 'no-user-doc' };
  if (userSnap.data().isPrimaryProducer === true) return { updated: false, reason: 'already-set-firestore' };

  await updateDoc(doc(db, 'users', adminId), {
    isPrimaryProducer: true,
    isEmployee: true,          // workspace owner is also the first employee
    role: 'producer',          // lock in the producer role
    updatedAt: serverTimestamp(),
  });

  return { updated: true, uid: adminId };
};
