...
export async function createTemplate(db, data, actorId) {
  return addDoc(collection(db, 'workflowTemplates'), {
    ...
    isActive: true,
    createdBy: actorId ?? null, // Firestore rejects `undefined` — was blocking ALL template saves
...
export async function createAssetRequest(db, projectId, data, actorId) {
  const ref = await addDoc(collection(db, 'projects', projectId, 'assetRequests'), {
    ...data,
    projectId,
    requestedBy: actorId,
  ...
  await postActivity(db, projectId, {
    type: 'asset-request.created',
    payload: { requestId: ref.id, title: data.title },
    actorId,
  });
  return ref.id;
}

export async function approveAssetRequest(db, projectId, requestId, actorId) {
  await updateDoc(doc(db, 'projects', projectId, 'assetRequests', requestId), {
    status: 'approved',
  });
  await postActivity(db, projectId, {
...
export async function fulfillAssetRequest(db, projectId, requestId, { notes, assetId }, actorId) {
  await updateDoc(doc(db, 'projects', projectId, 'assetRequests', requestId), {
    status: 'fulfilled',
  ...
  await postActivity(db, projectId, {
    type: 'asset-request.fulfilled',
    payload: { requestId, notes, assetId },
    actorId,
  });
}

export async function cancelAssetRequest(db, projectId, requestId, actorId) {
  await updateDoc(doc(db, 'projects', projectId, 'assetRequests', requestId), {
    status: 'cancelled',
  });
}

export async function getAssetRequests(db, projectId) {
  const snap = await getDocs(
    query(collection(db, 'projects', projectId, 'assetRequests'), orderBy('requestedAt', 'desc'))
  );
  return snap.docs.map(d => ({ ...d.data(), id: d.id }));
}
