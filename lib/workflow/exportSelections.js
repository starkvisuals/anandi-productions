...
/**
 * Generate CSV from a block's corrections array.
 * @param {object[]} corrections  — block.corrections array
 * @param {string} blockLabel     — e.g. "Client Approval V1"
 * @param {string} projectName
 * @returns {string}  UTF-8 BOM-prefixed CSV string
 */
export function correctionsToCSV(corrections, blockLabel, projectName) {
  const rows = [];
  ...
  if (rows.length === 0) {
    rows.push({ 'Round': '', 'Item #': '', 'Correction': 'No corrections recorded', 'Resolved': '', 'Submitted By': '', 'Submitted At': '', 'Block': blockLabel || '', 'Project': projectName || '' });
  }
  const csv = Papa.unparse(rows, { quotes: true });
