...
//
// Invariants (must be ensured by project creation and the runner):
// - project.shareToken MUST be set before any SelectionRound or DeliveryBlock
//   hook is invoked. These hooks emit a client-facing URL and do not guard
//   against missing tokens — a missing token produces `/share/undefined`.
//   B3 (project creation) sets it; A4 runner preconditions should assert it.
...
export const BLOCK_RULES = {
  ...
  [BLOCK_TYPES.SelectionRound]: {
    onEnter: ({ block, project }) => ({
      ...
      setSLAReminders: block.slaHours ? [
  ...
  [BLOCK_TYPES.ApprovalRound]: {
    onEnter: ({ block, project }) => ({
      ...
      setSLAReminders: block.slaHours ? [
        { atHoursFromStart: block.slaHours * 0.75, template: EMAIL_TEMPLATES.APPROVAL_REMINDER },
        { atHoursFromStart: block.slaHours, template: EMAIL_TEMPLATES.APPROVAL_OVERDUE, escalate: true },
      ] : [],
    }),
