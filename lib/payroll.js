...
export async function buildPayrollSheet(actor, month, { workingDays, excludeSundays = true } = {}) {
  ...
  const rows = employees
    ...
    // Exclude employees who left before this month started (their last working
    // day is before the 1st). Someone leaving mid-month is still paid this month.
    .filter(e => {
      const left = e.employmentStatus === 'terminated' || e.employmentStatus === 'resigned';
      if (!left) return true;
      return !e.lastWorkingDay || e.lastWorkingDay >= monthStart;
    })
