import { db } from './firebase';
import { collection, doc, getDoc, getDocs, setDoc, updateDoc, deleteDoc, query, where, serverTimestamp, runTransaction } from 'firebase/firestore';

export const generateId = () => Math.random().toString(36).substr(2, 9) + Date.now().toString(36);
export const generateShareToken = () => Math.random().toString(36).substr(2, 12) + Date.now().toString(36);

export const TEAM_ROLES = {
  'photo-editor': { label: 'Photo Editor', icon: 'PE', color: '#ec4899' },
  'video-editor': { label: 'Video Editor', icon: 'VE', color: '#f97316' },
  'colorist': { label: 'Colorist', icon: 'CL', color: '#a855f7' },
  'vfx-artist': { label: 'VFX Artist', icon: 'VX', color: '#10b981' },
  'sound-designer': { label: 'Sound Designer', icon: 'SD', color: '#06b6d4' },
  'motion-designer': { label: 'Motion Designer', icon: 'MD', color: '#6366f1' },
  'cgi-artist': { label: 'CGI Artist', icon: 'CG', color: '#3b82f6' },
};

export const CORE_ROLES = {
  'producer': { label: 'Producer', icon: 'PR', color: '#f59e0b' },
  'admin': { label: 'Admin', icon: 'AD', color: '#ef4444' },
  'team-lead': { label: 'Team Lead', icon: 'TL', color: '#8b5cf6' },
  'coordinator': { label: 'Coordinator', icon: 'CO', color: '#22c55e' },
  'agency': { label: 'Agency', icon: 'AG', color: '#0ea5e9' },
};

export const STATUS = {
  'pending': { label: 'Pending', bg: '#fef3c7', color: '#92400e' },
  'selected': { label: 'Selected', bg: '#dbeafe', color: '#1e40af' },
  'assigned': { label: 'Assigned', bg: '#e0e7ff', color: '#3730a3' },
  'in-progress': { label: 'In Progress', bg: '#fae8ff', color: '#86198f' },
  'review-ready': { label: 'Review Ready', bg: '#fef3c7', color: '#92400e' },
  'changes-requested': { label: 'Changes', bg: '#fee2e2', color: '#991b1b' },
  'agency-review': { label: 'Agency Review', bg: '#e0f2fe', color: '#0369a1' },
  'approved': { label: 'Approved', bg: '#d1fae5', color: '#065f46' },
  'delivered': { label: 'Delivered', bg: '#cffafe', color: '#155e75' },
};

export const createUser = async (uid, data) => { const ref = doc(db, 'users', uid); await setDoc(ref, { ...data, id: uid, createdAt: serverTimestamp() }); return { id: uid, ...data }; };
export const getUser = async (uid) => { const snap = await getDoc(doc(db, 'users', uid)); return snap.exists() ? { id: snap.id, ...snap.data() } : null; };
export const getUsers = async () => { const snap = await getDocs(collection(db, 'users')); return snap.docs.map(d => ({ id: d.id, ...d.data() })); };
export const updateUser = async (uid, data) => { await updateDoc(doc(db, 'users', uid), { ...data, updatedAt: serverTimestamp() }); };
export const deleteUser = async (uid) => { await deleteDoc(doc(db, 'users', uid)); };
export const getFreelancers = async () => { const q = query(collection(db, 'users'), where('isFreelancer', '==', true)); const snap = await getDocs(q); return snap.docs.map(d => ({ id: d.id, ...d.data() })); };
export const getClients = async () => { const q = query(collection(db, 'users'), where('isClient', '==', true)); const snap = await getDocs(q); return snap.docs.map(d => ({ id: d.id, ...d.data() })); };
export const getCoreTeam = async () => { const q = query(collection(db, 'users'), where('isCore', '==', true)); const snap = await getDocs(q); return snap.docs.map(d => ({ id: d.id, ...d.data() })); };

export const createProject = async (data) => { const id = generateId(); const ref = doc(db, 'projects', id); const proj = { ...data, id, shareLinks: [], createdAt: serverTimestamp() }; await setDoc(ref, proj); return proj; };
export const getProjects = async () => { const snap = await getDocs(collection(db, 'projects')); return snap.docs.map(d => ({ id: d.id, ...d.data() })); };
export const getProject = async (id) => { const snap = await getDoc(doc(db, 'projects', id)); return snap.exists() ? { id: snap.id, ...snap.data() } : null; };
export const updateProject = async (id, data) => { await updateDoc(doc(db, 'projects', id), { ...data, updatedAt: serverTimestamp() }); };
export const deleteProject = async (id) => { await deleteDoc(doc(db, 'projects', id)); };

// --- Concurrency-safe project writes -------------------------------------
// The whole app model stores assets + comments + selections as arrays inside
// the `projects/{id}` doc. A plain read-modify-write (`updateProject` after
// reading in the component) is LAST-WRITE-WINS: if two people act on the same
// project in the same moment, the second save silently clobbers the first
// (a lost comment / upload / status change). With multiple team members,
// clients and vendors reviewing at once, that is real data loss.
//
// `mutateProject` re-reads the freshest project INSIDE a Firestore transaction
// and applies your change on top of it; if someone else wrote a split-second
// earlier, Firestore retries the mutator against THEIR version instead of
// overwriting it. Same doc shape, no migration — just a safe write path.
//
//   await mutateProject(id, (proj) => ({ assets: [...proj.assets, newAsset] }));
//
// The mutator receives the current project ({id, ...data}) and returns a
// PARTIAL patch to merge, or a falsy value to abort the write (no-op).
export const mutateProject = async (id, mutator) => {
  const ref = doc(db, 'projects', id);
  return runTransaction(db, async (tx) => {
    const snap = await tx.get(ref);
    if (!snap.exists()) throw new Error('Project not found');
    const current = { id: snap.id, ...snap.data() };
    const patch = mutator(current);
    if (!patch) return current; // mutator aborted → leave the doc untouched
    tx.update(ref, { ...patch, updatedAt: serverTimestamp() });
    return { ...current, ...patch };
  });
};

// Atomically update ONE asset (by id) via a mutator on that asset. Concurrent
// edits to DIFFERENT assets never conflict; two edits to the SAME asset still
// resolve safely (the transaction re-runs on the latest version). Returns the
// updated project, or null if the asset no longer exists.
export const mutateAsset = async (projectId, assetId, assetMutator) => {
  return mutateProject(projectId, (proj) => {
    const assets = proj.assets || [];
    const idx = assets.findIndex((a) => a.id === assetId);
    if (idx === -1) return null;
    const nextAsset = assetMutator({ ...assets[idx] }, proj);
    if (!nextAsset) return null;
    const next = assets.slice();
    next[idx] = nextAsset;
    return { assets: next };
  });
};
export const getProjectsForUser = async (userId, role) => {
  if (['producer', 'admin', 'team-lead'].includes(role)) return getProjects();
  const all = await getProjects();
  return all.filter(p => p.assignedTeam?.some(t => t.odId === userId) || p.clientContacts?.some(c => c.odId === userId) || p.createdBy === userId);
};

// --- Share-token index ---------------------------------------------------
// `shareTokens/{token}` maps a share token -> its project, so a login-less
// client resolves their link with a single doc GET instead of listing (and
// downloading) the ENTIRE projects collection. This is what lets the Firestore
// rules block anonymous `list` on `projects` without breaking share links.
export const setShareTokenIndex = async (token, projectId, active = true) => {
  await setDoc(doc(db, 'shareTokens', token), { projectId, active, updatedAt: serverTimestamp() });
};

export const createShareLink = async (projectId, data) => {
  const token = generateShareToken();
  const link = { id: generateId(), token, projectId, ...data, createdAt: new Date().toISOString(), active: true };
  const proj = await getProject(projectId);
  await updateProject(projectId, { shareLinks: [...(proj.shareLinks || []), link] });
  await setShareTokenIndex(token, projectId, true);
  return link;
};

export const getProjectByShareToken = async (token) => {
  // Fast path: direct index lookup (no collection scan).
  try {
    const idxSnap = await getDoc(doc(db, 'shareTokens', token));
    if (idxSnap.exists()) {
      const { projectId, active } = idxSnap.data();
      if (active !== false && projectId) {
        const p = await getProject(projectId);
        const link = p && (p.shareLinks || []).find(l => l.token === token && l.active);
        if (link) return { project: p, link };
      }
    }
  } catch (e) { /* fall through to legacy scan */ }
  // Legacy fallback: tokens created before the index existed. Backfill the
  // index on the way through so the next lookup is fast + rules-safe.
  const all = await getProjects();
  for (const p of all) {
    const link = (p.shareLinks || []).find(l => l.token === token && l.active);
    if (link) {
      try { await setShareTokenIndex(token, p.id, true); } catch (e) { /* best effort */ }
      return { project: p, link };
    }
  }
  return null;
};

export const deactivateShareLink = async (projectId, linkId) => {
  const proj = await getProject(projectId);
  const target = (proj.shareLinks || []).find(l => l.id === linkId);
  await updateProject(projectId, { shareLinks: (proj.shareLinks || []).map(l => l.id === linkId ? { ...l, active: false } : l) });
  if (target?.token) { try { await setShareTokenIndex(target.token, projectId, false); } catch (e) { /* best effort */ } }
};

export const getSettings = async () => { const snap = await getDoc(doc(db, 'settings', 'app')); return snap.exists() ? snap.data() : null; };
export const saveSettings = async (data) => { await setDoc(doc(db, 'settings', 'app'), { ...data, updatedAt: serverTimestamp() }); };
