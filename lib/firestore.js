...
export const STATUS = {
  ...
  'in-progress': { label: 'In Progress', bg: '#e0e7ff', color: '#3730a3' },
  'review-ready': { label: 'Ready for Review', bg: '#fae8ff', color: '#86198f' },
  'changes-requested': { label: 'Changes Requested', bg: '#fee2e2', color: '#991b1b' },
  'approved': { label: 'Approved', bg: '#d1fae5', color: '#065f46' },
  'completed': { label: 'Completed', bg: '#cffafe', color: '#155e75' },
};
...
export const getProjectsForUser = async (userId, role, isClient, isFreelancer) => {
...
export const createShareLink = async (projectId, data) => {
  ...
  const links = [...(proj.shareLinks || []), link];
  await updateProject(projectId, { shareLinks: links });
  return link;
};
...
export const deactivateShareLink = async (projectId, linkId) => {
  const proj = await getProject(projectId);
  const links = (proj.shareLinks || []).map(l => l.id === linkId ? { ...l, active: false } : l);
  await updateProject(projectId, { shareLinks: links });
};
