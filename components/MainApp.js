...
import { getProjects, getProjectsForUser, createProject, updateProject, getUsers, getFreelancers, getClients, getCoreTeam, createUser, createShareLink, deactivateShareLink, TEAM_ROLES, CORE_ROLES, STATUS, generateId } from '@/lib/firestore';
...
const StarRating = ({ rating = 0, onChange, size = 16, readonly = false }) => {
  ...
  return <div style={{ display: 'flex', gap: '2px' }}>{[1,2,3,4,5].map(star => <span key={star} onClick={() => !readonly && onChange?.(star === rating ? 0 : star)} onMouseEnter={() => !readonly && setHover(star)} onMouseLeave={() => !readonly && setHover(0)} style={{ cursor: readonly ? 'default' : 'pointer', fontSize: size, color: star <= (hover || rating) ? '#fbbf24' : '#3a3a4a', transition: 'color 0.1s' }}>★</span>)}</div>;
};
...
export default function MainApp() {
  ...
  const ProjectDetail = () => {
    ...
    const handleSaveAnnotations = async (annotations) => {
      const updated = (selectedProject.assets || []).map(a => a.id === selectedAsset.id ? { ...a, annotations } : a);
      await updateProject(selectedProject.id, { assets: updated });
      await refreshProject();
      setSelectedAsset({ ...selectedAsset, annotations });
      ...
      <div style={{ display: 'flex', marginLeft: '-200px' }}>
        ...
        <div style={{ width: '180px', background: '#12121a', borderRight: '1px solid #1e1e2e', height: 'calc(100vh - 46px)', position: 'fixed', left: '200px', top: '46px', overflowY: 'auto', zIndex: 40 }}>
          <div style={{ padding: '10px' }}>
            ...
            <div onClick={() => setSelectedCat(null)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', borderRadius: '5px', cursor: 'pointer', fontSize: '11px', background: !selectedCat ? 'rgba(99,102,241,0.15)' : 'transparent', color: !selectedCat ? '#fff' : 'rgba(255,255,255,0.6)', marginBottom: '2px' }}><span>📁 All</span><span style={{ fontSize: '9px', opacity: 0.6 }}>{(selectedProject.assets || []).length}</span></div>
            {cats.map(cat => <div key={cat.id} onClick={() => setSelectedCat(cat.id)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', borderRadius: '5px', cursor: 'pointer', fontSize: '11px', background: selectedCat === cat.id ? 'rgba(99,102,241,0.15)' : 'transparent', color: selectedCat === cat.id ? '#fff' : 'rgba(255,255,255,0.6)', marginBottom: '2px' }}><span>{cat.icon} {cat.name}</span><span style={{ fontSize: '9px', opacity: 0.6 }}>{getCatCount(cat.id)}</span></div>)}
          </div>
          <div style={{ padding: '10px', borderTop: '1px solid #1e1e2e' }}>
            ...
            {[{ label: '⏳ Pending', f: a => a.status === 'pending' }, { label: '⭐ Selected', f: a => a.isSelected }, { label: '👀 Review', f: a => a.status === 'review-ready' }, { label: '✅ Approved', f: a => a.status === 'approved' }].map((f, i) => <div key={i} style={{ padding: '6px 10px', fontSize: '10px', color: 'rgba(255,255,255,0.5)' }}>{f.label} ({(selectedProject.assets || []).filter(f.f).length})</div>)}
          </div>
        </div>
        ...
        <div style={{ flex: 1, marginLeft: '380px' }}>
          ...
          <div style={{ padding: '8px 16px', borderBottom: '1px solid #1e1e2e', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {tab === 'assets' && (
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                ...
                {!selectedProject.selectionConfirmed && selectedCount > 0 && isProducer && <Btn onClick={handleConfirmSelection} small color="#f59e0b">🎯 Confirm ({selectedCount})</Btn>}
              </div>
            )}
          </div>
          ...
          {Object.keys(uploadProgress).length > 0 && (
            <div style={{ padding: '10px 16px', background: '#1e1e2e' }}>
              {Object.entries(uploadProgress).map(([id, item]) => <div key={id} style={{ marginBottom: '4px' }}><div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', marginBottom: '2px' }}><span>{item.name}</span><span>{item.progress}%</span></div><div style={{ background: '#0d0d14', borderRadius: '2px', height: '3px' }}><div style={{ width: `${item.progress}%`, height: '100%', background: '#6366f1', borderRadius: '2px' }} /></div></div>)}
            </div>
          )}
          ...
          <div style={{ padding: '16px' }}>
            ...
            {tab === 'assets' && (
              <div>
                {assets.length === 0 ? (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fill, minmax(${cardWidth}px, 1fr))`, gap: '12px' }}>
                    {assets.map(a => (
                      <div key={a.id} style={{ background: '#16161f', borderRadius: '8px', overflow: 'hidden', border: selectedAssets.has(a.id) ? '2px solid #6366f1' : '1px solid #1e1e2e', position: 'relative' }}>
                        <div onClick={e => { e.stopPropagation(); setSelectedAssets(s => { const n = new Set(s); n.has(a.id) ? n.delete(a.id) : n.add(a.id); return n; }); }} style={{ position: 'absolute', top: '8px', left: '8px', width: '18px', height: '18px', borderRadius: '4px', background: selectedAssets.has(a.id) ? '#6366f1' : 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 5 }}>{selectedAssets.has(a.id) && <span style={{ color: '#fff', fontSize: '10px' }}>✓</span>}</div>
                        {a.isSelected && <div style={{ position: 'absolute', top: '8px', right: '8px', background: '#22c55e', borderRadius: '4px', padding: '2px 5px', fontSize: '8px', zIndex: 5 }}>⭐</div>}
                        {a.annotations?.length > 0 && <div style={{ position: 'absolute', top: '8px', right: a.isSelected ? '50px' : '8px', background: '#f97316', borderRadius: '4px', padding: '2px 5px', fontSize: '8px', zIndex: 5 }}>✏️{a.annotations.length}</div>}
                        <div onClick={() => { setSelectedAsset(a); setAssetTab('preview'); }} style={{ cursor: 'pointer', height: `${cardWidth / aspectRatio}px`, background: '#0d0d14', position: 'relative' }}>
                          {a.type === 'video' ? <VideoThumbnail src={a.url} duration={a.duration} style={{ width: '100%', height: '100%' }} /> : a.type === 'audio' ? <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontSize: '30px' }}>🔊</span></div> : a.thumbnail ? <img src={a.thumbnail} alt="" style={{ width: '100%', height: '100%', objectFit: appearance.thumbScale === 'fill' ? 'cover' : 'contain' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontSize: '30px' }}>📄</span></div>}
                  ...
                  <div>
                    {assets.map(a => (
                      <div key={a.id} onClick={() => { setSelectedAsset(a); setAssetTab('preview'); }} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px', background: '#16161f', borderRadius: '8px', marginBottom: '6px', cursor: 'pointer', border: selectedAssets.has(a.id) ? '2px solid #6366f1' : '1px solid #1e1e2e' }}>
                        <div onClick={e => { e.stopPropagation(); setSelectedAssets(s => { const n = new Set(s); n.has(a.id) ? n.delete(a.id) : n.add(a.id); return n; }); }} style={{ width: '18px', height: '18px', borderRadius: '4px', background: selectedAssets.has(a.id) ? '#6366f1' : '#0d0d14', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>{selectedAssets.has(a.id) && <span style={{ color: '#fff', fontSize: '10px' }}>✓</span>}</div>
                        <div style={{ width: '50px', height: '50px', borderRadius: '6px', overflow: 'hidden', background: '#0d0d14', flexShrink: 0 }}>{a.thumbnail ? <img src={a.thumbnail} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{a.type === 'video' ? '🎬' : '📄'}</div>}</div>
                        <div style={{ flex: 1 }}><div style={{ fontWeight: '500', fontSize: '11px' }}>{a.name}</div><div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)' }}>{formatFileSize(a.fileSize)} • v{a.currentVersion}</div></div>
                        <StarRating rating={a.rating} onChange={r => { r.stopPropagation?.(); handleRate(a.id, r); }} size={14} />
                        <Badge status={a.status} />
            ...
            {tab === 'team' && (
              <div style={{ background: '#16161f', borderRadius: '10px', border: '1px solid #1e1e2e' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid #1e1e2e', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><h3 style={{ margin: 0, fontSize: '12px' }}>👥 Team ({team.length})</h3>{isProducer && availableTeam.length > 0 && <Btn onClick={() => setShowAddTeam(true)} small>+ Add</Btn>}</div>
                <div style={{ padding: '12px' }}>{team.length === 0 ? <div style={{ textAlign: 'center', padding: '20px', color: 'rgba(255,255,255,0.4)', fontSize: '11px' }}>No team</div> : team.map(m => <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', background: '#0d0d14', borderRadius: '8px', marginBottom: '6px' }}><Avatar user={m} size={34} /><div style={{ flex: 1 }}><div style={{ fontWeight: '500', fontSize: '11px' }}>{m.name} {m.isOwner && <span style={{ fontSize: '8px', color: '#f97316' }}>👑</span>}</div><div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)' }}>{m.email}</div></div><RoleBadge role={m.role} /></div>)}</div>
              </div>
            )}
            ...
            {tab === 'activity' && (
              <div style={{ background: '#16161f', borderRadius: '10px', border: '1px solid #1e1e2e', padding: '16px' }}>
                ...
                {(selectedProject.activityLog || []).length === 0 ? <div style={{ textAlign: 'center', padding: '20px', color: 'rgba(255,255,255,0.4)', fontSize: '11px' }}>No activity</div> : (selectedProject.activityLog || []).slice().reverse().map(log => <div key={log.id} style={{ display: 'flex', gap: '8px', padding: '8px', background: '#0d0d14', borderRadius: '6px', marginBottom: '5px' }}><div style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#6366f1', marginTop: '5px', flexShrink: 0 }} /><div><div style={{ fontSize: '11px' }}>{log.message}</div><div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)' }}>{formatTimeAgo(log.timestamp)}</div></div></div>)}
              </div>
            )}
            ...
            {tab === 'links' && (
              <div>
                ...
                <div style={{ background: '#16161f', borderRadius: '10px', border: '1px solid #1e1e2e', padding: '14px' }}>
                  ...
                  {shareLinks.length === 0 ? <div style={{ textAlign: 'center', padding: '20px', color: 'rgba(255,255,255,0.4)', fontSize: '11px' }}>No links</div> : shareLinks.map(link => <div key={link.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', background: '#0d0d14', borderRadius: '8px', marginBottom: '6px' }}><span style={{ fontSize: '16px' }}>{link.type === 'client' ? '👔' : '🎨'}</span><div style={{ flex: 1 }}><div style={{ fontWeight: '500', fontSize: '11px' }}>{link.name}</div><div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)' }}>{link.type} • {formatTimeAgo(link.createdAt)}</div></div><Btn onClick={() => copyLink(link.token)} small outline>📋 Copy</Btn></div>)}
                </div>
              </div>
            )}
          </div>
        ...
        {showAddTeam && (
          <Modal title="Add Team Member" onClose={() => setShowAddTeam(false)}>
            <div style={{ maxHeight: '350px', overflow: 'auto' }}>{availableTeam.length === 0 ? <div style={{ textAlign: 'center', padding: '20px', color: 'rgba(255,255,255,0.4)', fontSize: '11px' }}>All added</div> : availableTeam.map(u => <div key={u.id} onClick={() => handleAddTeam(u.id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', background: '#0d0d14', borderRadius: '8px', marginBottom: '6px', cursor: 'pointer' }}><Avatar user={u} size={36} /><div style={{ flex: 1 }}><div style={{ fontWeight: '500', fontSize: '11px' }}>{u.name}</div><div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)' }}>{u.email}</div></div><RoleBadge role={u.role} /></div>)}</div>
          </Modal>
        )}

        {/* Asset Preview Modal with Tabs */}
        {selectedAsset && (
          <Modal title={selectedAsset.name} onClose={() => setSelectedAsset(null)} wide>
            {/* Asset Tabs */}
            <div style={{ display: 'flex', gap: '4px', marginBottom: '14px', borderBottom: '1px solid #1e1e2e', paddingBottom: '10px' }}>
              {[{ id: 'preview', icon: '👁️', label: 'Preview' }, { id: 'annotate', icon: '✏️', label: 'Annotate' }, { id: 'compare', icon: '📊', label: 'Compare' }].map(t => (
                <button key={t.id} onClick={() => setAssetTab(t.id)} style={{ padding: '6px 14px', background: assetTab === t.id ? '#6366f1' : 'transparent', border: assetTab === t.id ? 'none' : '1px solid #2a2a3e', borderRadius: '5px', color: '#fff', fontSize: '11px', cursor: 'pointer' }}>{t.icon} {t.label}</button>
              ))}
            </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: '16px' }}>
                <div>
                  <div style={{ background: '#0d0d14', borderRadius: '10px', overflow: 'hidden', marginBottom: '14px' }}>
                    {selectedAsset.type === 'video' ? <video src={selectedAsset.url} controls style={{ width: '100%', maxHeight: '380px' }} /> : selectedAsset.type === 'audio' ? <div style={{ padding: '40px', textAlign: 'center' }}><div style={{ fontSize: '50px', marginBottom: '14px' }}>🔊</div><audio src={selectedAsset.url} controls style={{ width: '100%' }} /></div> : selectedAsset.type === 'image' ? <img src={selectedAsset.url} alt="" style={{ width: '100%', maxHeight: '380px', objectFit: 'contain' }} /> : <div style={{ padding: '40px', textAlign: 'center' }}><div style={{ fontSize: '50px' }}>📄</div></div>}
                  </div>
                  <div style={{ background: '#0d0d14', borderRadius: '10px', padding: '12px' }}>
                    ...
                    <div style={{ maxHeight: '120px', overflow: 'auto', marginBottom: '8px' }}>{(selectedAsset.feedback || []).length === 0 ? <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)' }}>No feedback</div> : (selectedAsset.feedback || []).map(fb => <div key={fb.id} style={{ padding: '6px', background: '#16161f', borderRadius: '5px', marginBottom: '4px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}><span style={{ fontSize: '10px', fontWeight: '500' }}>{fb.userName}</span><span style={{ fontSize: '8px', color: 'rgba(255,255,255,0.4)' }}>{formatTimeAgo(fb.timestamp)}</span></div><div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.8)' }}>{fb.text}</div></div>)}</div>
                ...
                <div>
                  <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '9px', color: 'rgba(255,255,255,0.4)', marginBottom: '4px' }}>Rating</label><StarRating rating={selectedAsset.rating} onChange={r => { handleRate(selectedAsset.id, r); setSelectedAsset({ ...selectedAsset, rating: r }); }} size={20} /></div>
                  <div style={{ marginBottom: '12px' }}><button onClick={() => { handleToggleSelect(selectedAsset.id); setSelectedAsset({ ...selectedAsset, isSelected: !selectedAsset.isSelected, status: !selectedAsset.isSelected ? 'selected' : 'pending' }); }} style={{ width: '100%', padding: '8px', background: selectedAsset.isSelected ? '#22c55e' : '#1e1e2e', border: 'none', borderRadius: '6px', color: '#fff', fontSize: '11px', cursor: 'pointer' }}>{selectedAsset.isSelected ? '⭐ Selected' : '☆ Mark as Selected'}</button></div>
                  ...
                  {isProducer && <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '9px', color: 'rgba(255,255,255,0.4)', marginBottom: '4px' }}>Assign To</label><Select value={selectedAsset.assignedTo || ''} onChange={v => handleAssign(selectedAsset.id, v)}><option value="">-- Unassigned --</option>{editors.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</Select></div>}
                  
                  {/* Version Upload */}
                  <div style={{ marginBottom: '12px', padding: '10px', background: '#0d0d14', borderRadius: '8px' }}>
                    <div style={{ fontSize: '10px', fontWeight: '600', marginBottom: '8px' }}>📦 Versions (v{selectedAsset.currentVersion})</div>
                    <div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)', marginBottom: '6px' }}>{(selectedAsset.versions || []).map(v => `v${v.version}`).join(', ')}</div>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <input ref={versionInputRef} type="file" style={{ display: 'none' }} onChange={e => setVersionFile(e.target.files?.[0] || null)} />
                      <button onClick={() => versionInputRef.current?.click()} style={{ flex: 1, padding: '6px', background: '#1e1e2e', border: '1px solid #2a2a3e', borderRadius: '4px', color: '#fff', fontSize: '10px', cursor: 'pointer' }}>{versionFile ? versionFile.name : '+ New Version'}</button>
                  ...
                  {selectedAsset.status === 'approved' && (
                    <div style={{ marginBottom: '12px' }}><label style={{ display: 'block', fontSize: '9px', color: 'rgba(255,255,255,0.4)', marginBottom: '4px' }}>📁 Google Drive Link</label><div style={{ display: 'flex', gap: '6px' }}><Input value={selectedAsset.gdriveLink || ''} onChange={v => setSelectedAsset({ ...selectedAsset, gdriveLink: v })} placeholder="Paste GDrive link" style={{ flex: 1 }} /><Btn onClick={() => handleSetGdriveLink(selectedAsset.id, selectedAsset.gdriveLink)} small>Save</Btn></div></div>
                  )}
                  {selectedAsset.gdriveLink && <a href={selectedAsset.gdriveLink} target="_blank" rel="noopener noreferrer" style={{ display: 'block', padding: '8px', background: 'rgba(34,197,94,0.15)', borderRadius: '6px', color: '#22c55e', fontSize: '11px', textAlign: 'center', textDecoration: 'none', marginBottom: '12px' }}>📁 Open High-Res</a>}
                  <div style={{ background: '#0d0d14', borderRadius: '6px', padding: '10px', marginBottom: '12px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px' }}><span style={{ color: 'rgba(255,255,255,0.5)' }}>Uploaded</span><span>{formatDate(selectedAsset.uploadedAt)}</span></div>
                  </div>
                  <a href={selectedAsset.url} download target="_blank" rel="noopener noreferrer" style={{ display: 'block', padding: '10px', background: '#6366f1', borderRadius: '6px', color: '#fff', fontSize: '11px', fontWeight: '600', textAlign: 'center', textDecoration: 'none' }}>⬇️ Download</a>
                </div>
              </div>
            )}
            ...
            {assetTab === 'annotate' && selectedAsset.type === 'image' && (
              <div>
                ...
                <div style={{ marginTop: '12px', padding: '10px', background: '#0d0d14', borderRadius: '8px' }}>
                  <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)' }}>Draw crop areas, circles, arrows, or add text. Annotations are saved automatically.</div>
                </div>
              </div>
            )}
            {assetTab === 'annotate' && selectedAsset.type !== 'image' && (
              <div style={{ padding: '50px', textAlign: 'center', background: '#0d0d14', borderRadius: '10px' }}>
                <div style={{ fontSize: '40px', marginBottom: '12px' }}>🎬</div>
                <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '12px' }}>Annotations are available for images only.<br/>For videos, use the feedback section with timestamps.</div>
              </div>
            )}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d0d14', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', color: '#e4e4e7' }}>
      ...
      <div style={{ marginLeft: '200px', minHeight: '100vh' }}>
        <div style={{ height: '46px', background: '#12121a', borderBottom: '1px solid #1e1e2e', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '0 16px', position: 'sticky', top: 0, zIndex: 50 }}><div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>{new Date().toLocaleDateString('en-IN', { weekday: 'short', month: 'short', day: 'numeric' })}</div></div>
        <div style={{ padding: '16px' }}>
