...
- [~] **A1.5** Wire into MainApp lightbox — **A1.5a DONE (live-test pending):** feedback panel → `<CommentSidebar>` with a feedback[]⇄sidebar adapter; side-effects preserved via `handleAddFeedback(text)`; compiles + boots. **A1.5b PENDING:** unify the annotate overlay (`AnnotationCanvas` → `ReviewCanvas` overlay-mode over the existing image/Mux player) + video-timeline markers + retire `handleSaveAnnotations`/`handleSaveVideoAnnotations`; needs ReviewCanvas BYO-media mode + live test. Then delete the now-dead old composer state/handlers.
