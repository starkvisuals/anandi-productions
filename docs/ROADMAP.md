...
- **LAST DONE (2026-09-15, cont.):** 🔔 **PIPELINE ① — stage notifications LIVE** (`0f7e609`). When a stage becomes someone's turn, the right person is finally told. **Root-cause fix:** the runner resolved email recipients from `project.teamMembers` (empty in this app — team lives in `project.assignedTeam` {odId,odRole}) → workflow emails reached NOBODY. Now `lib/workflow/runner.js` resolves from `assignedTeam→users` (workflow-role→app-role alias map: editor→[video-editor,photo-editor,...], producer→[producer,admin,team-lead], etc.) + `clientContacts`; every transition emails everyone with an address AND creates an in-app notification for internal users (clients/vendors = email only, no bell). Fail-safe: a non-client role matching nobody → producers (never silently un-notified). New `lib/notifications.js` = shared Firestore `notifications` store; MainApp bell now subscribes live + merges with the existing local deadline reminders (mark-one/all persist to Firestore, fail-soft if index missing). ⚠️ **HARNESH TODO (30s):** first bell load will need a Firestore composite index — click the auto-link the browser console prints, OR create in console → Firestore → Indexes: collection `notifications` (1) `userId` ASC + `createdAt` DESC, (2) `userId` ASC + `read` ASC. ⚠️ Full end-to-end verify needs a live project with a workflow + advancing a stage (tie to ③ seed pipelines).
  - **NEXT: ② hold-ups board** (producer screen: every active project, current stage, owner, on-time/due-soon/overdue) → then **③ auto-chase + due-soon delivery reminders** (the SLA descriptors already exist on blocks via `setSLAReminders`; needs a scheduled job to fire them) + **seed starter pipelines**.

- **LAST DONE (2026-09-15):** 🔒 **Concurrency-safe project writes** (`6620f66`). Harnesh confirmed the real usage: MANY people (team + clients + vendors) uploading/commenting/reviewing the SAME project simultaneously — so the PLAYBOOK's "revisit last-write-wins when multiple review at once" condition is now met. Added `mutateProject()`/`mutateAsset()` (lib/firestore) = read-modify-write inside a Firestore transaction (retries on freshest doc; no shape change/migration). Routed the hot paths through them in MainApp (uploads append, comments/replies/resolve append onto freshest feedback[], rate, toggle/bulk select, colour label, muxPlaybackId persist) AND wrapped the login-less client/vendor actions in the share `mutate` route in an Admin `runTransaction`. Compiles clean (3 files). ⚠️ Live-verify needs two simultaneous sessions (can't visually verify alone); single-user actions behave as before.
  - **NEXT (Harnesh's stated priority): the PIPELINE ENGINE** — he wants the software to RUN each workflow: at every stage notify the responsible party (team/vendor/client), set the stage deadline, auto-chase overdue, block the next stage until the current is approved, and a server-side "hold-ups" dashboard so he never manually checks who's lagging. Builder UI + STAGE_META already exist; the engine (materializeBlocksFromTemplate/advanceProject) is ROADMAP-flagged fragile (B2) — diagnose before extending. Pair with Deadline Reminders + branded client Delivery Room. STILL PENDING from before: whole-app polish (Review lightbox next), Mux webhook wire-up (#3), vendor access model clarification.
