...
| B2 | P0 | Video loads very slowly | **ROOT CAUSE FOUND:** the Mux **webhook is a no-op** (`app/api/mux/webhook/route.js` receives `video.asset.ready` + the playbackId and discards it), so `muxPlaybackId` is never saved → the player falls back to the **raw Firebase `.mp4`** (`src={muxPlaybackId ? undefined : url}`), no HLS, no storyboard. **INTERIM FIX DONE:** client auto-poll — opening a video without a playbackId now auto-fetches + persists it (MainApp, after the HLS effect), so it switches to fast Mux HLS on its own (self-heals; first view still ~one slow load). *Re-test.* **PROPER FIX (Harnesh):** provide Firebase Admin SDK creds (3 env vars) + set the Mux webhook URL in the dashboard → then wire the webhook to persist instantly for everyone. | ⚙️ interim fixed / webhook pending |
